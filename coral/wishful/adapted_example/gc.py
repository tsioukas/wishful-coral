...
def main(args):
    ...
    while True:
        ...
        if nodes:
	 #    print ("Testing...")
	    # apo to simple (ekteleitai se olous tous nodes)
            #global_control_engine.blocking(False).node(nodes[0]).radio.iface("wlan0").set_power(12)
            #global_control_engine.blocking(False).node(nodes[0]).net.get_authenticated_connected_device()
            
            #global_control_engine.node(nodes[0]).net.iface("wlan0").run_command("ping -c 2 www.google.com")
            
            #global_control_engine.node(nodes[0]).net.iface("wlan0").get_inactivity_time_of_connected_devices()
    
   
            
            #global_control_engine.blocking(False).node(nodes[0]).radio.iface("wlan0").get_channel()
            
            # not working..............
            #global_control_engine.blocking(False).node(nodes[0]).radio.iface("wlan0").get_iface_ip_addr()
            
           
            global_control_engine.blocking(False).node(nodes[0]).net.iface("eth0").get_iface_ip_addr("eth0")
            
            
            global_control_engine.blocking(False).node(nodes[0]).radio.iface("wlan0").get_power()
            #global_control_engine.blocking(False).node(nodes[1]).radio.iface("wlan0").get_power()
            global_control_engine.blocking(False).node(nodes[0]).radio.iface("wlan0").set_power(5)
            #global_control_engine.blocking(False).node(nodes[1]).radio.iface("wlan0").set_power(9)
            
            #global_control_engine.blocking(False).node(nodes[1]).radio.iface("wlan0").get_channel()
            
            
            
            #global_control_engine.blocking(False).node(nodes[0]).radio.iface("wlan0").set_channel(4)
