...
def calcStationsPDR (packetSendTotal):
	...
	try:
		...
		if ( packetSendTotal>0 ):
			...
			write2logfile( createLogFile4PDR(), justPDRline, str(datetime.datetime.now()))
			#create JSON
		...
		try:
			...
			if (getRes):	
				...
				if n == 2:				
					...
					print ("\n"+line2write ) # print ONLY ONE NODE
				write2logfile (coralLogFile, line2write, str(datetime.datetime.now()))
		except Exception as e:
...
def write2logfile (logfileName, line2write, time):
	...
	try:
		with open('./logs/'+str(logfileName), "a") as myfile:
...
# ========== Changing send interval every half hour ==============================
			half_hour_check = time.time() - half_hour_counter
			print ("half_hour_check : {}".format(half_hour_check))
			
			if ( half_hour_check > 1800 ): # half hour = 30mins*60 = 1800
				print("Half hour reset...")
				half_hour_counter =  time.time() # reset 
